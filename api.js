export async function calcularGastosDetalhados(pet) {
  ...
  if (!racaoMensal) {
    if (idade <= 1) racaoBase *= 1.15;
    else if (idade > 8) racaoBase *= 0.85;
    switch (porte) {
    ...
    if (tipo.includes('sementes')) racaoBase *= 0.5;
  ...
  if (!vacinasAnual) {
    if (especie === 'cachorro') vacinasAnual = idade < 1 ? 350 : 180;
  ...
  const freqBanho = pet.banhoFreq ? pet.banhoFreq : 'mensal'; 
  let banhoFreqMult = 1;
